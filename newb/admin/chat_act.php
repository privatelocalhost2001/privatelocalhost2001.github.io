...
if($_POST['action'] == 'update_user_list') {
	$chatUsers = $chat->chatUsers($rowUser['customer_id']);
	$data = array(
	...
	$chat->insertChat($_POST['to_user_id'], $rowUser['customer_id'], $_POST['chat_message'], $rowSender['cust_photo']);
...
if($_POST['action'] == 'show_chat') {
	$chat->showUserChat($rowUser['customer_id'], $_POST['to_user_id']);
}
if($_POST['action'] == 'update_user_chat') {
	$conversation = $chat->getUserChat($rowUser['customer_id'], $_POST['to_user_id']);
	$data = array(
...
if($_POST['action'] == 'update_unread_message') {
	$count = $chat->getUnreadMessageCount($_POST['to_user_id'], $rowUser['customer_id']);
	$data = array(
		"count" => $count			
	);
	echo json_encode($data);
...
$create = $conn->query("insert into chat_table (chat_id, chat_status, user_id, from_id, to_id, cust_name, email, chat_content, media, chats_date) values (NULL, '1', '".$rowUser['customer_id']."', '". $rowUser['customer_id'] ."', '". $to_id ."', 'ADMIN', 'ADMINEMAIL', '" . $content ."', '', '" . $date . "')") or die($conn->error);
