...
$create = $conn->query("insert into send_info (info_id, info_status, accnt_nmb, info_sbj, info_msg, info_date) values (NULL, '1', '" . $rowInfo['acct_nmb'] . "', '" . $_POST['info_sbj'] . "', '" . $_POST['info_msg'] . "', '" . date("d M Y, g:i a") . "')") or die($conn->error);
    
header('location:send_information.php?msg=Notification update  successfully for '.$rowInfo['cust_name'] . '&id=' .$whoto);    
}
...
header("location:send_information.php?msg=Notification information  has been successfully removed&id=". $_GET['iddd']);
}
...
<div class="row">
        ...
        <li class="btn btn-warning-off"><button  ><div><a href="send_mass_information.php">SEND MASS INFORMATION =></a></div></button></li>
                

              <?php
                $seLp = $conn->query("select * from customer_info where customer_id = '" . $_GET['id'] . "'") or die($conn->error); 
                $rowp = $seLp->fetch_array(); 
                ?>
                
                <button  class="btn btn-warning-off" ><div align="right" class="pull-right" ><a href="more_info.php?id=<?php echo $rowp['customer_id']; ?>">Back To User Info =></a></div></button>
                
              <li class="pull-left header"><i class="fa fa-inbox"></i>Email Dashboard of <strong><?php echo $rowp['cust_name'] ?></strong> </li>
            </ul>
        ...
        <form id="form1" name="form1" method="post" action="send_information.php">  
            ...
            <div>
              ...
              <tr>
<td><input name="USER-ID" type="hidden" value="<?php echo $rowp['customer_id']; ?>"  size="30" class="form-control"  title="Add a message subject" /></td>
            </tr>
    ...
    <button  class="pull-right btn btn-default" type="submit"   name="SendThisUserNote" value="sendEmail" >Send
                <i class="fa fa-arrow-circle-right"></i></button>
                  ...
                  <?php
				  $i=0;
				  $seLNInfo = $conn->query("select * from send_info where accnt_nmb = '" . $rowp['acct_nmb'] . "'") or die($conn->error);
...
function clickedTrans(e)
{
  ...
  <footer class="main-footer">
    ...
    <strong>Copyright &copy; 2016-2017 <a href="https://adminlte.io">bank</a>.</strong> All rights
    reserved.
  </footer>
