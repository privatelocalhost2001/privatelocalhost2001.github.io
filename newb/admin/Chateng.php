...
class Chat{
	...
	public function chatUsers($customer_id){
		$sqlQuery = "
			...
			WHERE customer_id != '$customer_id' ORDER BY cust_name ASC";
		return  $this->getData($sqlQuery);
	}
	...
	public function insertChat($to_id, $MY_ID, $chat_message, $my_photo) {	
      ...
      (chat_id, chat_status, user_id, from_id, to_id, cust_name, cust_photo, email, chat_content, media, chat_date) 
			VALUES   (NULL, '1', '".$MY_ID."', '". $MY_ID ."', '". $to_id ."', 'ADMIN', '".$my_photo."', 'ADMINEMAIL', '" . $chat_message ."', '', '" . date("d M Y, g:i a") . "')";
...
<li class="dropdown messages-menu" title="settings" style="width: 130px">
        <a href="#" class="dropdown-toggle dropdown-toggle11" data-toggle="dropdown" >
              <i class="glyphicon glyphicon-tasks"></i>          
        </a>

       <ul class="dropdown-menu" style="width: 100px"  >
            <li >
                 <ul title="Clear User Chat" >
                    <a href="chat.php?CID=' .$user['customer_id'].'&DELCHA=Delete chat">
                       <i class="glyphicon glyphicon-trash" aria-hidden="true" style="font-size: 11px; position: ; top: 5; left: 0; ><a href="">Clear Chat</a></i>
                    </a>
                 </ul>
             </li>
       </ul>
	...
	public function getUnreadMessageCount($senderUserid, $recieverUserid) {
		$sqlQuery = "
			SELECT * FROM ".$this->chatTable."  
			WHERE from_id = '$senderUserid' AND to_id = '$recieverUserid' AND chat_status = '1'";
	...
	public function updateTypingStatus($is_type, $customer_id) {		
		$sqlUpdate = "
			UPDATE ".$this->chatUsersTable." 
	...
	public function fetchIsTypeStatus($customer_id){
		...
		WHERE customer_id = '".$customer_id."' ORDER BY email DESC LIMIT 1"; 
